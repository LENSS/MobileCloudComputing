	...
	public void run() {
		while (keepRunning) {
			try {
				...
				Logger.v(TAG, "Receive Connection");
			...
			while (keepBroadcasting) {
				try {
					...
					blockingIps = Node.getInstance().getRouteManager()
							.getBlockingIpSet();
					if (blockingIps.contains(bdNodeId)) {
						continue;
					}

					bdResult = new byte[brodcastReceivePacket.getLength()];
					System.arraycopy(brodcastReceivePacket.getData(), 0,
							bdResult, 0, brodcastReceivePacket.getLength());

					// Logger.v(TAG, "Receive Broadcast of " + result.length +
					// " bytes from " + nodeId);
